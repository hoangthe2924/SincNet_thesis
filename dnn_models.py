    ...
    def forward(self, x):
        ...
        for i in range(self.N_fc_lay):
            ...
            if self.fc_act[i] != "linear":
                ...
                divider_number = 1
                if self.fc_act[i] == "tanh":
                    divider_number = 10

                if self.fc_use_laynorm[i]:
                    x = self.drop[i](self.act[i](self.ln[i](self.wx[i](x)) / divider_number))

                if self.fc_use_batchnorm[i]:
                    x = self.drop[i](self.act[i](self.bn[i](self.wx[i](x)) / divider_number))

                if self.fc_use_batchnorm[i] == False and self.fc_use_laynorm[i] == False:
                    x = self.drop[i](self.act[i](self.wx[i](x) / divider_number))
